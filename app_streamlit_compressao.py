# app_streamlit_compressao.py - Simulador de Ensaio de Compressão em Streamlit
...
MATERIAIS = {
    "Aço": {"E": 200e9, "limite_escoamento": 250e6, "ruptura": 400e6, "deform_max": 0.10, "n": 0.2, "K": 800e6, "escoamento_duracao": 0.01},
    "Alumínio": {"E": 70e9, "limite_escoamento": 150e6, "ruptura": 300e6, "deform_max": 0.08, "n": 0.15, "K": 500e6, "escoamento_duracao": 0.005},
    "Cobre": {"E": 110e9, "limite_escoamento": 210e6, "ruptura": 350e6, "deform_max": 0.15, "n": 0.25, "K": 650e6, "escoamento_duracao": 0.0},
}
cores = {"Aço": "blue", "Alumínio": "green", "Cobre": "red"}
Lo = 50  # mm (altura inicial do corpo de prova)

# Geração da curva compressiva

def curva_compressao(material):
...
if st.button("Iniciar Ensaio"):
    epsilon, delta_L, tensao, deltaL_elastico, deltaL_plato, deltaL_ruptura, tensao_ruptura = curva_compressao(material)
    cor = cores[material]
    ...
    for i in range(1, n_frames + 1):
        ...
        ax1.text(0.05 * deltaL_ruptura, tensao_ruptura * 1.1, f"Tensão atual: {tensao[i-1]:.1f} MPa", fontsize=10, bbox=dict(facecolor='white'))
        ax1.text(0.05 * deltaL_ruptura, tensao_ruptura * 1.0, f"∆L atual: {delta_L[i-1]:.2f} mm", fontsize=10, bbox=dict(facecolor='white'))

        ax1.legend(loc='lower right')

        # Corpo de prova comprimido
